...
When a scheduled time arrives, Posthook makes an HTTP POST request to your destination.

The destination URL is constructed by combining your **Project Domain** (configured in settings) with the **Path** specified when scheduling the hook.

> `api.myapp.com` (Domain) + `/webhooks/remind` (Path) = `https://api.myapp.com/webhooks/remind` (HTTPS is enforced)

## The Request

Posthook sends a `POST` request with a wrapper object containing your data and metadata about the scheduling.

```json
{
  "id": "hook_01H...",
  "path": "/webhooks/remind",
  "postAt": "2024-01-01T12:00:00Z",
  "postedAt": "2024-01-01T12:00:00.123Z",
  "data": {
    "user_id": "usr_123",
    "reminder_id": "rem_456"
  },
...
If your endpoint returns a non-200 status code, or times out (timeout is 10 seconds), Posthook will retry the delivery based on your project's retry configuration.
...
Posthook has a strict **10-second timeout** for webhook delivery. If your task takes longer than 10 seconds (e.g., generating a PDF, processing a video), your request will time out and be retried.
...
To ensure that the request comes from Posthook and not a malicious actor, you must verify the signature.
